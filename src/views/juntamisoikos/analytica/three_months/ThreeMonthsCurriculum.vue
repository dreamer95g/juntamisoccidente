  ...
  <div class="mx-4 my-4 mt-4">
    <div class="p-6 bg-white rounded-md shadow-lg">
      <h2 class="text-lg text-gray-700 font-semibold capitalize" v-if="!loading">
        {{ title }} {{ curriculum_name }}
      </h2>
      <br />
      <div v-if="!loading">
        <div class="flex-auto flex space-x-2">
          <div>
            <label class="text-gray-700">Curriculum:</label>
            <div>
              <ThreeAutoComplete
                @curricula="name = $event"
                query="curricula"
                :default="name"
...
export default Vue.extend({
  data: () => {
    return {
      ...
      //atributos propios
      loading: false,
      title: "Informe Trimestral Curriculum",
      placeholder: "Curriculum",
      name: "",
      curriculum_name: "",
      curriculum_id: "",
  ...
  methods: {
    ...
    controller: function () {
      if (this.name !== "" && this.threeSelected !== "" && this.year !== "") {
        ...
        this.notify("warning", "Atencion", "Llene correctamente los campos");
      }
    },
    ...
    getResults: async function (id, three_selected, year) {
      await this.$apollo
        ...
        .then((data) => {
          //console.log(data.data.threeMonthsCurriculum);
          let result = data.data.threeMonthsCurriculum;
    ...
    async getId(name) {
      await this.$apollo
        ...
        .then((data) => {
          //console.log(data.data.curriculumWhere);
          this.curriculum_id = data.data.curriculumWhere.id;
        })
        .catch((error) => {
          //console.error(error);
          this.notify("error", "Error", "No se encontro ningun elemento");
